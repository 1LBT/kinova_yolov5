...
def execute_grasp_sequence():
    ...
    q = tft.quaternion_from_euler(np.pi, 0, 0)
    ...
    rospy.loginfo(f"移动到预抓取点 (Z_wrist={pre_grasp_z:.3f})...")
    if not move_to_pose_with_retry(pre_grasp_pos, orientation):
        MOVING = False
        return False
